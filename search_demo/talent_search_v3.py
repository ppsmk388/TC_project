# -*- coding: utf-8 -*-
"""
talent_search_langgraph_v4.py
SearXNG + LangGraph + LLM (engine router, self-correcting query optimizer)
Pipeline:
  parse_query → enrich_field → plan → choose_engines → search → select → fetch
  → seed_from_sources → candidate_enrich → profile_norm → coauthor_expand
  → student_gate → synthesize → (optimizer_if_needed → loop) → end

Key features:
- Name+Paper enrichment & LLM query optimizer
- Per-query engine routing (google/scholar/arxiv/github/…)
- Robust homepage ranking & profile lift (OpenReview + Semantic Scholar JSON-LD)
- Student-status gate (edu/email/affiliation signals) with conservative fallback
- Tight context budgeting to avoid 4k ctx overflow
- Self-correction: optimizer proposes new search terms if target not met
"""

import os, re, io, sys, json, time, html, contextlib, datetime
...
SAVE_DIR = "./result_save"; os.makedirs(SAVE_DIR, exist_ok=True)

# ---- LLM: read from env for safety; falls back to your defaults if set ----
LOCAL_OPENAI_BASE_URL = os.getenv("LLM_BASE_URL", "http://localhost:8000/v1")
LOCAL_OPENAI_MODEL    = os.getenv("LLM_MODEL",    "Qwen3-8B")
LOCAL_OPENAI_API_KEY  = os.getenv("LLM_API_KEY",  "sk-local")  # set env var!
...
# ---- tight context budgets (≈ 4k ctx models) ----
...
DEFAULT_ENGINES  = "google"  # router will pick per-query; fallback to this
SEARXNG_PAGES    = 2

# ---- Engine router switch ----
...
UA = {"User-Agent": "Mozilla/5.0 (TalentSearch-LangGraph-v4)"}
...
FIELD_ONTOLOGY = {
    "social_sim": {
        ...
        "synonyms": ["social simulation","multi-agent","llm agents","agent-based","agentic","behavior modeling","agent simulation"],
    },
    "nlp": {"venues":["ACL","EMNLP","NAACL","COLING","TACL"],
            "synonyms":["natural language","language modeling","llm","text generation","machine translation"]},
    "ml_general": {"venues":["ICLR","ICML","NeurIPS","AISTATS"],
                   "synonyms":["foundation model","pretraining","self-supervised","representation learning"]},
}
...
LLM_OUT_TOKENS = {
    ...
    "qopt":20000,"synthesize":20000,"router":20000,"optimizer":20000
}
...
def get_llm(role: str, temperature: float = 0.4):
    return ChatOpenAI(
        ...
        max_tokens=LLM_OUT_TOKENS.get(role, 800),
    )
...
class PlanSpec(BaseModel):
    ...
    engine_map: Dict[str, List[str]] = Field(default_factory=dict)      # per-query engines
    bad_terms: List[str] = Field(default_factory=list)                   # avoid repeats
...
def build_conference_queries(spec: QuerySpec, default_confs: Dict[str, List[str]], cap: int = 180) -> List[str]:
...
def engine_heuristic(q: str) -> List[str]:
    ...
    if "github" in ql or "site:github" in ql: return ["github", "google"]
    if "wikipedia" in ql or "wikidata" in ql: return ["wikipedia", "wikidata"]
    if "scholar" in ql or "site:scholar.google.com" in ql: return ["google scholar", "google"]
    return ["google"]
...
def llm_choose_engines(queries: List[str]) -> Dict[str, List[str]]:
    ...
    sample = queries[:60]  # context cap
    prompt = (
        "You are a search-engine router for SearXNG. "
        "For each query, pick 1-2 engines from this set:\n"
        f"{allow}\n\n"
        "Heuristics: paper db → google scholar; preprints → arxiv; DOI → crossref; code → github; facts → wikipedia/wikidata; general → google.\n"
        "Return STRICT JSON: {\"routes\":[{\"q\":\"...\",\"engines\":[\"google\",\"arxiv\"]}, ...]}\n\n"
...
def _route_after_synthesize(state: ResearchState) -> str:
    if (state.round + 1) >= MAX_ROUNDS: return "end"
    if state.need_more: return "loop"
    return "end"

# ============================ NODES ============================

def _node_wrapper(node_name: str, node_func):
    """Wrapper to show which node is currently executing"""
    def wrapped(state: ResearchState) -> Dict[str, Any]:
        print(f"\n[EXEC] → {node_name}")
        print(f"  State: round={state.round}, serp={len(state.serp)}, sources={len(state.sources)}, candidates={len(state.candidates)}")
        try:
...
def node_enrich_field(state: ResearchState) -> Dict[str, Any]:
    ...
    plan.setdefault("visited_authors", []); plan.setdefault("frontier", {}); plan.setdefault("bad_terms", [])
    return {"query_spec": spec.model_dump(), "plan": plan}

def node_plan(state: ResearchState) -> Dict[str, Any]:
    spec = QuerySpec.model_validate(state.query_spec)
    terms = build_conference_queries(spec, DEFAULT_CONFERENCES, cap=180)
    # drop banned/bad terms if any
    bad = set(state.plan.get("bad_terms", []))
    terms = [t for t in terms if t not in bad]
    plan = PlanSpec(
        search_terms=terms,
...
def node_search(state: ResearchState) -> Dict[str, Any]:
    ...
    for term in terms:
        engines_list = engine_map.get(term, []) or [DEFAULT_ENGINES]
...
def llm_query_optimize(name: str, seed_papers: List[str], existing_links: Dict[str,str]) -> List[str]:
    ...
    prompt = (
        "Given a candidate and missing profile fields, propose up to 6 precise web search queries. "
...
def node_profile_normalize_and_rank(state: ResearchState) -> Dict[str, Any]:
    ...
    for author in list(frontier.keys())[:60]:
...
# ---------- self-correcting optimizer (adds new search terms if needed) ----------
def node_optimizer(state: ResearchState) -> Dict[str, Any]:
    """LLM inspects what we have and proposes new, non-duplicate search terms.
       It also bans low-yield/bad terms seen so far to avoid loops."""
    if not state.need_more:
        return {}
    llm = get_llm("optimizer", temperature=0.2)
    spec = QuerySpec.model_validate(state.query_spec)
    plan = dict(state.plan)
    have = [c.get("Name","") for c in state.candidates][:spec.top_n]
    used_terms = plan.get("search_terms", [])[:140]
    bad = set(plan.get("bad_terms", []))
    prompt = (
        "We are assembling student candidates from top ML/NLP venues. "
        "Given our partial shortlist and the search terms used, propose up to 15 NEW high-yield queries to find more candidates. "
        "Focus on: accepted/program pages of the right venues/years; and targeted queries that reveal author profiles for this topic.\n"
        f"Topic keywords: {', '.join(spec.keywords) or 'LLM agents, multi-agent, social simulation'}\n"
        f"Venues: {', '.join(spec.venues)} Years: {spec.years}\n"
        f"Already have {len(have)} candidates: {have}\n"
        "Rules:\n- Return STRICT JSON {\"queries\":[\"...\"]}\n- Avoid duplicates of the list below; avoid weak generic queries.\n"
        f"- Avoid these (bad_terms): {list(bad)}\n"
        f"- Prior used terms (dedupe against these): {used_terms[:60]}"
    )
    qspec = safe_structured(llm, prompt, QOptSpec)
    new = []
    seen = set(used_terms)
    for q in (qspec.queries or []):
        q = re.sub(r"\s+", " ", q).strip()
        if not q: continue
        if q in seen: continue
        seen.add(q); new.append(q)
        if len(new) >= 15: break
    # simple ban of low-yield patterns
    ban_add = [t for t in used_terms if re.search(r"\b(schedule|call for papers|camera ready)\b", t, re.I)]
    bad.update(ban_add)
    # update plan
    plan["search_terms"] = list(dict.fromkeys(used_terms + new))
    plan["bad_terms"] = list(bad)
    if VERBOSE:
        print(f"[optimizer] added {len(new)} new queries; bad_terms size={len(bad)}")
    return {"plan": plan, "need_more": False}  # we'll re-evaluate need_more after next synth
...
def build_graph():
    ...
    # Add nodes with execution tracking
    g.add_node("parse_query", _node_wrapper("parse_query", node_parse_query))
    g.add_node("enrich_field", _node_wrapper("enrich_field", node_enrich_field))
    g.add_node("plan", _node_wrapper("plan", node_plan))
    g.add_node("choose_engines", _node_wrapper("choose_engines", node_choose_engines))
    g.add_node("search", _node_wrapper("search", node_search))
    g.add_node("select", _node_wrapper("select", node_select))
    g.add_node("fetch", _node_wrapper("fetch", node_fetch))
    g.add_node("seed_from_sources", _node_wrapper("seed_from_sources", node_seed_from_sources))
    g.add_node("candidate_enrich", _node_wrapper("candidate_enrich", node_candidate_enrich))
    g.add_node("profile_norm", _node_wrapper("profile_norm", node_profile_normalize_and_rank))
    g.add_node("coauthor_expand", _node_wrapper("coauthor_expand", node_coauthor_expand))
    g.add_node("student_gate", _node_wrapper("student_gate", node_student_status_gate))
    g.add_node("synthesize", _node_wrapper("synthesize", node_synthesize))
    g.add_node("optimizer", _node_wrapper("optimizer", node_optimizer))
    g.add_node("inc_round", _node_wrapper("inc_round", node_inc_round))
    ...
    g.add_edge("plan", "choose_engines")
    ...
    # If synth says need_more, run optimizer to add more queries, then loop
    def route_after_synthesize(state: ResearchState) -> str:
        decision = "loop" if (state.need_more and (state.round + 1) < MAX_ROUNDS) else "end"
        print(f"\n[ROUTING] synthesize → {decision}")
        print(f"  need_more: {state.need_more}, round: {state.round}, MAX_ROUNDS: {MAX_ROUNDS}")
        if decision == "loop":
            print(f"  → Will loop: optimizer → inc_round → plan (round {state.round + 1})")
        else:
            print(f"  → Will end: reached MAX_ROUNDS or no need_more")
        return decision
    
    g.add_conditional_edges(
        "synthesize",
        route_after_synthesize,
        {"loop": "optimizer", "end": END}
    )
    g.add_edge("optimizer", "inc_round")
    ...
    print("="*60)
    print("Entry point: parse_query")
    print("\nLinear flow:")
    print("parse_query → enrich_field → plan → choose_engines → search → select → fetch")
    print("→ seed_from_sources → candidate_enrich → profile_norm → coauthor_expand → student_gate → synthesize")
    print("\nConditional routing from synthesize:")
    print("  - If need_more=True AND round < MAX_ROUNDS: synthesize → optimizer → inc_round → plan (LOOP)")
    print("  - If need_more=False OR round >= MAX_ROUNDS: synthesize → END")
    print("\nLoop path:")
    print("optimizer → inc_round → plan → choose_engines → search → select → fetch → ...")
    print("="*60)
...
def talent_search(question: str, ts: Optional[str] = None) -> Dict[str, Any]:
    ...
    print(f"[cfg] LLM model={LOCAL_OPENAI_MODEL} base={LOCAL_OPENAI_BASE_URL}")
    print(f"[cfg] ROUNDS={MAX_ROUNDS} SELECT_K={SELECT_K} SEARCH_K={SEARCH_K}")

    app = build_graph(); init = ResearchState(query=question)
    final = app.invoke(init); st = _ensure_state(final); spec = QuerySpec.model_validate(st.query_spec)
    ...
    print("="*60)
    print(f"Total rounds executed: {st.round + 1}")
    print(f"Final state: need_more={st.need_more}, candidates={len(st.candidates)}")
    print(f"Graph nodes visited: parse_query → enrich_field → plan → choose_engines → search → select → fetch")
    print(f"→ seed_from_sources → candidate_enrich → profile_norm → coauthor_expand → student_gate → synthesize")
    if st.round > 0:
        print(f"Loop executed {st.round} times: optimizer → inc_round → plan → ...")
    print("="*60)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Talent Search via SearXNG + LangGraph (v4, self-correcting)")
    parser.add_argument("question", nargs="*", help="your talent-scouting query")
    args = parser.parse_args()
    default_q = (
        "I am a recruiter from MSRA. Identify 10 rising-star interns who are current PhD or Master's students working on social simulation with LLMs or multi-agent systems. "
        "Focus on candidates with recent publications at ACL, EMNLP, NAACL, NeurIPS, ICLR, ICML or on OpenReview."
    )
    q = " ".join(args.question) if args.question else default_q
